...
STRICT_READONLY_AUDIT_AGENTS=("citation-verifier" "coverage-critic" "summary-auditor" "novelty-checker" "concept-auditor")
...
is_strict_readonly_audit_agent() {
  local name="$1"
  for a in "${STRICT_READONLY_AUDIT_AGENTS[@]}"; do
    [[ "$name" == "$a" ]] && return 0
  done
  return 1
}
...
for f in "$AGENT_DIR"/*.md; do
  ...
  if is_audit_agent "$base"; then
    ...
    if is_strict_readonly_audit_agent "$base"; then
    ...
    if ! grep -q '^fallback_model:' <<<"$fm"; then
      echo "FAIL [C1] $base: audit agent must declare fallback_model"
      FAIL=$((FAIL+1))
    fi
  fi
done
